  ...
  describe('Fluxo Oficial de Cadastro — Server Action register()', () => {
    ...
    it('register() com dados válidos cria usuário via Server Action (fluxo oficial)', async () => {
      ...
      const tenantUserRecord = {
        ...
        role: 'admin',
        createdAt: new Date(),
      } as Awaited<ReturnType<typeof prisma.tenantUser.create>>;
      const emailVerificationRecord = {
        ...
        createdAt: new Date(),
      } as Awaited<ReturnType<typeof prisma.emailVerificationToken.create>>;
      ...
      vi.mocked(prisma.emailVerificationToken.create).mockResolvedValue(emailVerificationRecord);
